...
class KNearestNeighbor(object):
    ...
    def compute_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            # 计算第 i 个测试点与所有训练点的欧几里得距离
            diff = X[i] - self.X_train  # 形状为 (num_train, D)
            dists[i, :] = np.sqrt(np.sum(diff ** 2, axis=1))  # 沿着每行求和，然后取平方根
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            # 找到与测试点i的距离最近的k个训练点的索引
            # closest_y_indices = np.argsort(dists[i])[:k]
            # closest_y = self.y_train[closest_y_indices]  # 获取这k个最近邻的标签
